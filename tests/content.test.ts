...
test('every concept has a definition, specific visual, and deeper references',()=>{
 for(const concept of concepts){assert.ok(concept.body.includes('### What it means'));assert.ok(visualSteps[concept.title]?.length>=3,concept.title);const graphic=conceptVisual(concept.title);assert.match(graphic,/<svg[^>]+role="img"/,concept.title);assert.match(graphic,/<desc>[^<]+<\/desc>/,concept.title);assert.match(graphic,/<(?:path|rect|circle) /,concept.title);assert.ok(!graphic.includes('NaN'),concept.title);assert.ok(deeperResources(concept.module).every(r=>(r.internal?/^[\w-]+\.html/.test(r.url):r.url.startsWith('https://'))&&r.purpose.length>20));}
});
