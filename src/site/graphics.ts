...
const text = (x:number,y:number,label:string) => `<text x="${x}" y="${y}">${esc(label)}</text>`;
const line = (x:number,y:number,a:number,b:number,extra='') => `<path d="M${x} ${y} L${a} ${b}" class="wire" ${extra}/>`;
const node = (x:number,y:number,label='') => `<circle cx="${x}" cy="${y}" r="13" class="neuron"/>${label?text(x,y+5,label):''}`;
const box = (x:number,y:number,w:number,h:number,label:string) => `<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="7" class="block"/>${text(x+w/2,y+h/2+5,label)}`;
function frame(label:string,body:string,description:string):string {
 return `<svg class="mechanism-graphic" viewBox="0 0 600 280" role="img" aria-label="${esc(label)}"><title>${esc(label)}</title><desc>${esc(description)}</desc>${body}</svg>`;
}
export function networkGraphic(width=4):string {
 const count=Math.min(width,8), layers=[2,count,2], positions=layers.map((n,i)=>Array.from({length:n},(_,j)=>[90+i*210,65+j*145/Math.max(1,n-1)]));
 let body='';
 for(let i=0;i<2;i++)for(const [x,y] of positions[i])for(const [a,b] of positions[i+1])body+=line(x,y,a,b);
 positions.forEach((layer,i)=>{layer.forEach(([x,y])=>body+=node(x,y));body+=text(90+i*210,245,['2 input features',`${width} hidden neurons`,'2 output scores'][i]);});
 if(width>8)body+=text(300,30,`8 of ${width} neurons shown`);
...
export function curveGraphic(overfit=false):string {
 const points=Array.from({length:41},(_,i)=>`${60+i*12},${overfit?65+130*(1-Math.exp(-i/9)):210-0.085*(i-20)**2}`).join(' ');
 let body=line(60,35,60,220)+line(60,220,550,220)+`<polyline class="curve" points="${points}"/>`;
 if(overfit)body+=`<path class="comparison" d="M60 65 Q180 240 360 130 T540 50"/>`+text(350,45,'Gold: held-out loss');
 else body+=node(180,125)+line(195,145,250,193)+node(300,210)+text(300,50,'L(w) = (w − 3)²');
...
export function transformerGraphic():string {
 return frame('Decoder transformer pathway',gridBody()+box(150,90,115,65,'Attention')+line(115,122,150,122)+line(265,122,310,122)+box(310,90,125,65,'Feed-forward')+line(435,122,480,122)+box(480,90,95,65,'Logits')+`<path class="comparison" d="M130 122 V55 H285 V122 M290 122 V195 H455 V122"/>`+text(300,245,'Gold paths: residual additions · norms omitted'),'Token embeddings pass through attention and feed-forward blocks with residual connections, then a vocabulary projection. Normalization and repeated blocks omitted.');
}
...
export function tokenGraphic():string {
 return frame('Text to token embeddings',text(300,30,'Text → token IDs → learned vectors')+['the','cat','sat'].map((t,i)=>box(45,65+i*60,100,40,t)+line(145,85+i*60,225,85+i*60)+node(245,85+i*60,String(i+7))+line(258,85+i*60,325,85+i*60)+Array.from({length:6},(_,j)=>`<rect x="${330+j*33}" y="${65+i*60}" width="28" height="40" class="signal" opacity="${.25+((i+j)%4)*.2}"/>`).join('')).join('')+text(300,270,'Illustrative word tokens and IDs; real tokenizers vary'),'Three word tokens map to integer IDs and rows of a learned embedding table. Shading represents illustrative feature values.');
}
...
export function stateGraphic(sharded=false):string {
 ...
 return frame('Distributed model state',body+text(300,255,sharded?'Bright tiles: locally owned state shards':'Bright tiles: replicated parameter state'),sharded?'Different workers own different parts of model state. Communication is required for computation.':'Workers replicate model state and process different local data.');
}
